...
function sort_by_vect(data::Vector{T}, order::Vector{T}) where T
  index = Vector{Int64}();
  for o in order
    aux = findall(data.==o);
    if !isempty(aux)
      index = vcat(index,aux);
    end
  end
  idx = findall(x->!(x in index),eachindex(data))
  return vcat(index,idx);  
end
...
function read_all_chunk(file::IOStream)
  ...
  has_PLTE,has_IDAT,has_mIDAT = false,false,false; 
   while true
    ...
    elseif output[end].type == "IDAT"
      if !has_IDAT
        has_IDAT=true
        if has_PLTE && output[end-1].type !="PLTE"
  ...
  order =["IHDR","PLTE","IDAT"]
  if !has_mIDAT
    return output[sort_by_vect(getfield.(output,:type),order)];
  end
...
function convert_to_myftm(chunks::Vector{Chunk})
  IHDR = chunks[1];
  IDAT = chunks[2];
  w,h,bit_depth,ct,cm,fm,im  = IHDR.data  
  # decompression
  if cm !=0
    error("compression method not supported, aborted")
  end
  d_data = decompression(IDAT.data);
end
...
for filename in ARGS 
  ...
  chunks = read_PNG(filename);
  convert_to_myftm(chunks);
end
